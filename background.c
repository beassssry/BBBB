...
void myexec(const char *cmd,const  char *resvec) 
{
    ...
    if (!pp)
 {
    ...
    while (fgets(tmp, sizeof(tmp), pp) != NULL) //读取管道
	{
        if (tmp[strlen(tmp) - 1] == '\n') 
	{
...
main()
{
    ...
    while(1)
    {
        ...
        receive_msg(&rev_msg,pro_path); //消息的接受
        myexec(rev_msg,&resvec);//地址赋予
        printf("%s\n",resvec);//打印容器的内容
        write_pipe(loadpath(pro_path,"lx_pipe"),resvec);//向管道中写入数据
